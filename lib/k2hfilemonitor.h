...
// member is for checking lastest process. Other two arrays are for 
...
// is changed, this process must reload the file. The same way, if mmap 
...
// often occurred, and all process is only reading flag. So the process 
...
class K2HFileMonitor
{
	...
	private:
		...
		bool InitializeFileMonitor(const SFMONWRAP* pfmonwrap, bool noupdate);
